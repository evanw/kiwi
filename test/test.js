...
it('struct int64', function () {
  function check(i, o) {
    assert.deepEqual(Buffer.from(schema.encodeInt64Struct({ x: i })), Buffer.from(o));
  ...
  check(0x4407_0C14_2030_4040n, [0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88]);
  check(-0x1000_0000_0000_0001n, [0x81, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x20]);
  check(0x1000_0000_0000_0001n, [0x82, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x20]);
  check(-0x3FFF_FFFF_FFFF_FFFFn, [0xFD, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x7F]);
  check(0x3FFF_FFFF_FFFF_FFFFn, [0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x7F]);
  check(-0x4000_0000_0000_0000n, [0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x7F]);
  check(0x4000_0000_0000_0000n, [0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80]);
  check(-0x7FFF_FFFF_FFFF_FFFFn, [0xFD, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF]);
  check(0x7FFF_FFFF_FFFF_FFFFn, [0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF]);
  check(-0x8000_0000_0000_0000n, [0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF]);
});

it('struct uint64', function () {
  function check(i, o) {
    assert.deepEqual(Buffer.from(schema.encodeUint64Struct({ x: i })), Buffer.from(o));
  ...
  check(0x7FFF_FFFFn, [0xFF, 0xFF, 0xFF, 0xFF, 0x07]);
  check(0x8000_0000n, [0x80, 0x80, 0x80, 0x80, 0x08]);
  check(0x880E_1828_4060_8080n, [0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88]);
  check(0x1000_0000_0000_0001n, [0x81, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x10]);
  check(0x7FFF_FFFF_FFFF_FFFFn, [0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x7F]);
  check(0x8000_0000_0000_0000n, [0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80]);
  check(0xFFFF_FFFF_FFFF_FFFFn, [0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF]);
});
...
it('message int64', function () {
  ...
  check({}, [0]);
  check({ x: 123456789012345678n }, [0x01, 0x9C, 0xCD, 0x87, 0xE3, 0xF4, 0xD2, 0xCD, 0xB6, 0x03, 0x00]);
  check({ x: -123456789012345678n }, [0x01, 0x9B, 0xCD, 0x87, 0xE3, 0xF4, 0xD2, 0xCD, 0xB6, 0x03, 0x00]);
});

it('message uint64', function () {
  function check(i, o) {
    assert.deepEqual(Buffer.from(schema.encodeUint64Message(i)), Buffer.from(o));
    assert.deepEqual(schema.decodeUint64Message(new Uint8Array(o)), i);
  }

  check({}, [0]);
  check({ x: 123456789012345678n }, [0x01, 0xCE, 0xE6, 0xC3, 0xB1, 0xBA, 0xE9, 0xA6, 0xDB, 0x01, 0x00]);
});
